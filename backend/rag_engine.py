...
# Prevent Keras 3 / TensorFlow import error — we only use PyTorch
os.environ["USE_TF"] = "0"
...
# Using lightweight embedding model for free-tier deployment (~30MB vs 100MB)
EMBEDDING_MODEL = os.getenv("EMBEDDING_MODEL", "sentence-transformers/all-TinyRoBERTa-L6-v2")
...
ANSWER_GENERATION_PROMPT = PromptTemplate.from_template(
    ...
    "Answer the user's question based on the information provided in the context. "
    "If the question asks for a summary or general idea (e.g., 'what is this about?'), "
    "synthesize a helpful answer from the provided chunks. "
    "If the information needed to answer the question is truly completely missing and unrelated, "
    "politely say 'I cannot find the answer in the provided document.'\n\n"
    "Recent Chat Context:\n{history}\n\n"
    "Document Context:\n{context}\n\n"
    "Question: {question}"
)
...
# Replaces langchain.retrievers.EnsembleRetriever which was removed in v1.2+
# =============================================================================
class SimpleEnsembleRetriever:
    """
    Combines BM25 (keyword) + FAISS (semantic) retriever results
    using Reciprocal Rank Fusion (RRF).
    """
    def __init__(self, retrievers: list, weights: list[float], k: int = 5):
    ...
    def invoke(self, query: str) -> list[Document]:
        ...
        all_docs_per_retriever = []
        for retriever in self.retrievers:
            try:
                docs = retriever.invoke(query)
            except Exception:
                docs = retriever.get_relevant_documents(query)
            all_docs_per_retriever.append(docs)
        ...
        for retriever_idx, docs in enumerate(all_docs_per_retriever):
            ...
            for rank, doc in enumerate(docs):
                ...
                doc_scores[doc_key] += weight * (1.0 / (rank + rrf_constant))
...
def build_retriever(chunks: list[Document]):
    """Build FAISS + BM25 retrievers, then wrap in SimpleEnsembleRetriever."""
